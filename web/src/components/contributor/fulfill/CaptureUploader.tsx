...
export function CaptureUploader() {
  const { setFiles, setDataHash, setSignature } = useFulfill();
  const [uppy] = useState(
    () =>
      new Uppy({
        autoProceed: false,
  ...
  useEffect(() => {
    // Changing the selection invalidates any hash and signature already produced.
    const sync = () => {
  ...
  }, [uppy, setFiles, setDataHash, setSignature]);
